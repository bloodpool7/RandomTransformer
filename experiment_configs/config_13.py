...
def get_config(basepath):
    ...
    config= {
        # wd
        'wd': os.path.join(basepath, 'experiments', 'test_12_size_1024_4'),
        # Raw data
...
def run_augmentation(basepath, queues):
    ...
    auged_data.extend(aug_scripts.aug_frequency(queues, queues_percent=45))
    auged_data.extend(aug_scripts.aug_block(queues, queues_percent=45))
