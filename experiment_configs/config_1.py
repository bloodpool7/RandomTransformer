...
def get_config(basepath):
    ...
    config= {
        ...
        # tests
        'tests': ['Frequency', "BlockFrequency", "Runs", "LongestRun", "Rank", "CumulativeSums", "FFT", "NonOverlappingTemplate", "Universal"],
...
def run_augmentation(basepath, queues):
    ...
    auged_data.extend(aug_scripts.aug_frequency(queues, queues_percent=100, visualize=True))
    auged_data.extend(aug_scripts.aug_block(queues, queues_percent=100, visualize=True))
    auged_data.extend(aug_scripts.aug_block_runs(queues, queues_percent=100))
    auged_data.extend(aug_scripts.aug_longest_run(queues, queues_percent=100))
    auged_data.extend(aug_scripts.aug_aperiodic_templates(queues, template_path, queues_percent=100))
